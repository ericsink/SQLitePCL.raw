...
public static class gen
{
	...
	private static void gen_csproj(config_csproj cfg, string root, string top, string libRoot = null)
	{
				...
				throw new NotImplementedException(string.Format("{0}, {1}", cfg.env, cfg.what));
					...
					case "watchos":
                        if (cfg.what == "e_sqlite3")
                        {
                            ...
                            f.WriteStartElement("ManifestResourceWithNoCulture");
                            // TODO underscore
                            f.WriteAttributeString("Include", Path.Combine(libRoot, "apple\\libs\\watchos\\sqlite3\\e_sqlite3.a"));
                            f.WriteElementString("Link", "e_sqlite3.a");
                            f.WriteEndElement(); // ManifestResourceWithNoCulture
                            f.WriteEndElement(); // ItemGroup
                        }
            else if (cfg.what == "sqlcipher")
                        {
                            ...
                            // TODO warning says this is deprecated
                            f.WriteStartElement("ManifestResourceWithNoCulture");
                            f.WriteAttributeString("Include", Path.Combine(libRoot, "couchbase-lite-libsqlcipher\\libs\\watchos\\libsqlcipher.a"));
                            f.WriteElementString("Link", "libsqlcipher.a");
                            f.WriteEndElement(); // ManifestResourceWithNoCulture
	...
	public static void Main(string[] args)
	{
		...
		{
			var name = args[0];
			var libRoot = args.Length > 1 ? args[1] : null;		// for bundles
			
			writePinvoke(cs_pinvoke, name, top);

			var customBuild = new CustomBuild(name, libRoot);
