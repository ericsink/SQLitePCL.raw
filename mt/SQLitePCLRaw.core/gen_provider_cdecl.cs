...
// TODO shouldn't there be a impl/bridge thing here
	...
	NativeMethods.callback_commit commit_hook_bridge = new NativeMethods.callback_commit(commit_hook_bridge_impl); 
        ...
        {
			if (db.commit_hook != null)
			{
                db.commit_hook.Dispose();
                db.commit_hook = null;
			}

            if (func != null)
            {
				db.commit_hook = new commit_hook_info(func, v);
                NativeMethods.sqlite3_commit_hook(db, commit_hook_bridge, db.commit_hook);
            }
            else
            {
                NativeMethods.sqlite3_commit_hook(db, null, null);
            }
        }
	...
	static class MyDelegateTypes
	{
		...
		[UnmanagedFunctionPointer(CALLING_CONVENTION)]
		public delegate IntPtr sqlite3_commit_hook(sqlite3 db, NativeMethods.callback_commit func, commit_hook_info pvUser);
